...
def mock_prompt_manager() -> MagicMock:
    """PromptManager stub — load() returns a canned summary prompt."""
    pm = MagicMock()
    pm.load = AsyncMock(return_value="STUB ARCHIVIST SUMMARY PROMPT")
    return pm
...
async def test_archivist_writes_frame_through_writer_not_supabase_insert(
    ...
    await archivist_agent.handle(
        request_obj, mock_router_for_summary, sb, mock_writer, mock_prompt_manager
    )
...
async def test_archivist_leaves_frame_in_cold_on_writer_failure(
    ...
    result = await archivist_agent.handle(
        request_obj, mock_router_for_summary, sb, mock_writer, mock_prompt_manager
    )
...
async def test_archivist_contemplator_handoff_writes_pattern_through_writer(
    ...
    await archivist_agent.handle(req, mock_router_for_summary, sb, mock_writer, mock_prompt_manager)
...
async def test_archivist_loads_summary_prompt_via_prompt_manager(
    request_obj: InvokeRequest,
    mock_router_for_summary: MagicMock,
    mock_writer: MagicMock,
    cold_frame_row: dict[str, Any],
    ...
    """Archivist's cold-frame migration must call prompt_manager.load("archivist", ...)
    before invoking the model — proves the centralized prompt path fires."""
    sb = MagicMock()
    sb.select = AsyncMock(return_value=[cold_frame_row])
    sb.delete = AsyncMock(return_value=None)
