...
# Detects session restart when SESSION_ID_FILE is absent (watchdog has exited).
#
...
SESSION_ID_FILE="$SESSION_DIR/session_id_${PROJECT}_${AGENT}"

# Ensure session dir exists
mkdir -p "$SESSION_DIR"

# Session restart detection — if SESSION_ID_FILE is missing, watchdog has exited
# This means Maxwell re-engaged after a session close without a fresh session start
if [ ! -f "$SESSION_ID_FILE" ]; then
  echo "$(date +%s)" > "$SESSION_ID_FILE"
  echo 0 > "$COUNTER_FILE"
  bash "$HAMMERFALL_DIR/scripts/brain.sh" "$PROJECT" "$AGENT" "scratchpad" \
    "SESSION RESTART — detected by ping_session.sh. No active session found. Counter reinitialised. Helm should execute Routine 0." false
fi
