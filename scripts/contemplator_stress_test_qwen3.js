...
function readConfig() {
  const brainUrl = process.env.SUPABASE_BRAIN_URL
  if (!brainUrl) throw new Error('SUPABASE_BRAIN_URL is not set.')
  const serviceKey = process.env.SUPABASE_BRAIN_SERVICE_KEY
  if (!serviceKey) throw new Error('SUPABASE_BRAIN_SERVICE_KEY is not set.')
  return { brainUrl: brainUrl.replace(/\/$/, ''), serviceKey }
}

async function fetchBrainData(
  brainUrl,
  serviceKey,
  { memCount = 10, beliefCount = 8, entityCount = 8 } = {}
) {
  ...
  const [mems, beliefs, entities] = await Promise.all([
    ...
    fetch(
      `${brainUrl}/rest/v1/helm_beliefs?active=eq.true&order=created_at.desc&limit=${beliefCount}&select=id,domain,belief,strength`,
      { headers: h }
    ).then((r) => r.json()),
    fetch(
...
function formatSnapshot(data) {
  return [
    ...
    '\n## Active Beliefs',
    data.beliefs
      .map((b) => `[id=${b.id}] [${b.domain}, strength=${b.strength}] ${b.belief}`)
      .join('\n'),
    '\n## Known Entities',
    data.entities
...
function assessPass1(data, snapshot) {
  ...
  if (typeof data.reflection_seed !== 'string' || data.reflection_seed.length < 10)
    issues.push('reflection_seed too short')
  ...
  if (verbatim.length > 0)
    issues.push(`regurgitation: ${verbatim.length} pattern(s) lifted verbatim`)
...
function assessPass2(data) {
  ...
  if (!data.reflection || typeof data.reflection.content !== 'string')
    issues.push('reflection missing')
  if (data.reflection?.content && data.reflection.content.split(/\s+/).length < 20)
    issues.push('reflection too short (<20 words)')
  if (data.reflection?.content && data.reflection.content.split(/\s+/).length > 120)
    issues.push('reflection too long (>120 words)')
  return issues
}
...
function assessPass3(data, snapshot) {
  ...
  if (!['intact', 'degraded', 'collapsed'].includes(data.context_quality))
    base.push('context_quality invalid')
  return base
}
...
async function runModelMode(mm, stdSnapshot, bigSnapshot) {
  ...
  try {
    const resp = await ollamaChat(
      ...
      [
        ...
        { role: 'user', content: PASS1_USER(stdSnapshot) },
      ],
  ...
  try {
    const resp = await ollamaChat(
      ...
      [
        ...
        { role: 'user', content: PASS2_USER(stdSnapshot, p1Data) },
      ],
  ...
  if (!p2Data) {
    ...
    if (p2Issues.length === 0) result.pass++
      ...
      result.fail++
      result.issues.push(...p2Issues.map((i) => `Pass 2: ${i}`))
    }
  }
  ...
  try {
    const resp = await ollamaChat(
      ...
      [
        { role: 'system', content: PASS1_SYSTEM },
        { role: 'user', content: PASS3_USER(bigSnapshot) },
      ],
  ...
  if (!p3Data) {
    ...
    if (p3Issues.length === 0) result.pass++
      ...
      result.fail++
      result.issues.push(...p3Issues.map((i) => `Pass 3: ${i}`))
    }
  }
...
async function main() {
  ...
  const stdData = await fetchBrainData(brainUrl, serviceKey, {
    memCount: 10,
    beliefCount: 8,
    entityCount: 8,
  })
  const bigData = await fetchBrainData(brainUrl, serviceKey, {
    memCount: 20,
    beliefCount: 15,
    entityCount: 15,
  })
  ...
  for (const mm of MODEL_MODES) {
    ...
    console.log(
      `  Pass 1: ${result.issues.some((i) => i.startsWith('Pass 1')) ? 'FAIL' : 'PASS'}  (${p1t.toFixed(1)}s)${thinkFlag}`
    )
    console.log(
      `  Pass 2: ${result.issues.some((i) => i.startsWith('Pass 2')) ? 'FAIL' : 'PASS'}  (${p2t.toFixed(1)}s)`
    )
    console.log(
      `  Pass 3: ${result.issues.some((i) => i.startsWith('Pass 3')) ? 'FAIL' : 'PASS'}  (${p3t.toFixed(1)}s)  context_quality=${cq}`
    )
  ...
  console.log('╚══════════════════════════════════════════════════════════════════╝')
  console.log(
    `  ${'Model / Mode'.padEnd(26)} ${'P1'.padEnd(6)} ${'P2'.padEnd(6)} ${'P3'.padEnd(6)} ${'Passes'.padEnd(8)} Avg(s)  ContextQ`
  )
  ...
  for (const mm of MODEL_MODES) {
    ...
    const total = 3
    const avg =
      r.times.filter((t) => t > 0).length > 0
        ? (
            r.times.filter((t) => t > 0).reduce((a, b) => a + b, 0) /
            r.times.filter((t) => t > 0).length /
            1000
          ).toFixed(1)
        : '—'
    const cq = r.p3?.context_quality || '—'
    console.log(
      `  ${mm.key.padEnd(26)} ${p1ok.padEnd(6)} ${p2ok.padEnd(6)} ${p3ok.padEnd(6)} ${pct(r.pass, total).padEnd(8)} ${avg}    ${cq}`
    )
  }
  ...
  for (const tier of SIZE_TIERS) {
    ...
    console.log(
      `  ${tier.label.padEnd(16)} ${baseStr.padEnd(20)} ${q3nStr.padEnd(18)} ${q3tStr.padEnd(18)} ${winner}`
    )
  }
  ...
  if (best) {
    console.log(
      `  Best: ${best.mm.label} — ${pct(best.r.pass, 3)} pass rate, context_quality=${bestCQ}`
    )
