...
const PASS1_USER_TEMPLATE = (
  snapshot
) => `Analyze the following brain snapshot for Helm (an AI agent system).
...
function entryText(entry) {
  ...
  if (entry && typeof entry === 'object') {
    return (
      entry.pattern || entry.description || entry.text || entry.summary || JSON.stringify(entry)
    )
  }
  return String(entry)
}
...
function assessPass1(data) {
  ...
  if (!Array.isArray(data.patterns) || data.patterns.length === 0)
    issues.push('patterns: empty or missing')
  if (!Array.isArray(data.gaps) || data.gaps.length === 0) issues.push('gaps: empty or missing')
  if (!['high', 'medium', 'low'].includes(data.signal_strength))
    issues.push('signal_strength: invalid value')
...
async function main() {
  ...
  const available = tags.models?.some(
    (m) => m.name === CONTEMPLATOR_MODEL || m.name.startsWith('qwen2.5:3b')
  )
  if (!available) {
    console.error(
      `FATAL: ${CONTEMPLATOR_MODEL} not found in Ollama. Run: ollama pull ${CONTEMPLATOR_MODEL}`
    )
    process.exit(1)
  }
  ...
  console.log(
    `  Memories: ${stdData.memories.length}, Beliefs: ${stdData.beliefs.length}, Entities: ${stdData.entities.length}`
  )
