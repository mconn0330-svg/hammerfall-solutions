...
Notice: You are operating in a Hammerfall project repo.
Adhere to these rules at all times.

## 1. Infrastructure & Environment
- Local-First: all active development uses a local Supabase/Docker instance.
  Do NOT provision cloud Supabase. Cloud triggers on first merge to main.
- Zero plain-text keys. All credentials via .env.local and GitHub Secrets.
...
- replit/ui-v1: Replit production frontend. FE Dev adopts components from here directly. UX Lead issues adoption report. Do not modify this branch — it is Replit's output.
- feature/[name]: only on explicit user request.

## 3. Pull Request Standards
Tag @Helm on every PR against develop.
Be prepared for the 3-Round Debate on architecture.

## 4. Chain of Command
- PM: central orchestrator. Breaks down PRDs into tasks.
- Doers (FE, BE, UX, QA): wait for PM task assignment.
- Status updates: update the task file in specs/ and ping PM.
