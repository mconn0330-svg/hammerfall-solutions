...
export default function EntitiesWidget() {
  ...
  const filtered = ENTITIES.filter(e =>
    !search || e.name.toLowerCase().includes(search.toLowerCase()) || e.entity_type.includes(search)
  )
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
                    <span style={{
                      fontFamily: 'var(--sans)', fontSize: 10, padding: '2px 8px', borderRadius: 4,
                      color: tc.color, background: tc.bg, border: `1px solid ${tc.color}33`, letterSpacing: '0.06em',
          ...
          <motion.div key="detail" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div style={{ padding: '16px 18px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                <span style={{ ...TYPE_COLOR[entity.entity_type], fontFamily: 'var(--sans)', fontSize: 10, padding: '2px 8px', borderRadius: 4, border: `1px solid ${(TYPE_COLOR[entity.entity_type]?.color || '#4db8ff')}33`, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                  {entity.entity_type}
                </span>
              ...
              <p className="body-text-sm" style={{ color: 'var(--text-secondary)', marginBottom: 14 }}>
                {entity.summary}
              </p>
              {entity.aliases.length > 0 && (
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
                  {entity.aliases.map(a => (
                    <span key={a} style={{ fontFamily: 'var(--sans)', fontSize: 10, padding: '2px 8px', borderRadius: 4, border: '1px solid var(--border)', color: 'var(--text-dim)' }}>{a}</span>
                  ))}
                </div>
              )}
              <p style={{ fontFamily: 'var(--sans)', fontSize: 10, color: 'var(--text-dim)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
