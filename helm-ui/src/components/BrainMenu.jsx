...
const ITEM_STYLE = {
  contemplate: { lineColor: '#f59e0b', labelColor: '#fbbf24', strokeW: 2.5, glow: 'url(#bm-glow-amber)' },
}
const DEFAULT_STYLE = { lineColor: '#4db8ff', labelColor: '#3a6a8a', strokeW: 1.8, glow: 'url(#bm-glow)' }
const ACTIVE_STYLE  = { lineColor: '#7dd4fc', labelColor: '#a8d8ff', strokeW: 2.5, glow: 'url(#bm-glow)' }
...
function rayDist(ox, oy, angle, rects, vpW, vpH) {
  ...
  for (const r of rects) {
    ...
    if (Math.abs(dx) < 1e-9) {
      ...
      const t1 = (r.x - ox) / dx, t2 = (r.x + r.w - ox) / dx
    ...
    if (Math.abs(dy) < 1e-9) {
      ...
      const t1 = (r.y - oy) / dy, t2 = (r.y + r.h - oy) / dy
      tmin = Math.max(tmin, Math.min(t1, t2))
      tmax = Math.min(tmax, Math.max(t1, t2))
    }
...
function placeLabelAngles(items, cx, cy, widgetRects, vpW, vpH, naturalLengths) {
  const claimed    = []
  const result     = {}
  const barObstacle = { x: 0, y: 0, w: vpW, h: BAR_HEIGHT + 4 }
  ...
  for (const item of items) {
    ...
    for (const cand of candidates) {
      ...
      const gutterBox = { x: realBox.x - G, y: realBox.y - G, w: realBox.w + G * 2, h: realBox.h + G * 2 }
...
// ─── Per-item tentacle ────────────────────────────────────────────────────────
function Tentacle({
  item, cx, cy, targetLen, open, travelProgress,
  isActive, isContempl, onSelect,
  vpW, vpH,
  labelAngle, labelHidden,
  isHovered, dimmed, onHover,
}) {
  const st        = isContempl ? ITEM_STYLE.contemplate : isActive ? ACTIVE_STYLE : DEFAULT_STYLE
  const gradId    = isContempl ? 'url(#sprite-amber)' : isActive ? 'url(#sprite-blue-active)' : 'url(#sprite-blue)'
  ...
  const barMaxLen = sinA < -1e-9
    ? Math.max(0, (cy - BAR_HEIGHT - 28) / Math.abs(sinA))
    : Infinity
  ...
  const ly       = Math.max(VP_PAD + BAR_HEIGHT, Math.min(cy + Math.sin(dispRad) * labelDist, vpH - VP_PAD))
        ...
        {open && travelProgress > 0.8 && (
          ...
          >
            ...
            <circle
              ...
              fill={isContempl ? 'url(#sprite-amber)' : isActive ? 'url(#sprite-blue-active)' : 'url(#sprite-blue)'}
            />
            <circle cx={ex} cy={ey} r={isContempl ? 3.5 : isActive ? 2.8 : 2.2} fill={coreColor} opacity={0.95} />
            {/* Pulsing ring — discovery hint when label is hidden */}
            {labelHidden && !isHovered && (
              <circle cx={ex} cy={ey} r={18} fill="none"
                stroke={isContempl ? '#f59e0b' : '#4db8ff'} strokeWidth={1.2}>
                <animate attributeName="r" values="15;26;15"
                  dur={`${pulseDur + 0.4}s`} repeatCount="indefinite" begin={`-${phaseOff * 0.5}s`} />
  ...
  return (
    ...
    >
      ...
      {hoveredIsHidden && open && (
        <rect
          x={0} y={BAR_HEIGHT} width={vpW} height={vpH - BAR_HEIGHT}
