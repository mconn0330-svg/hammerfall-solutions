...
// ─── WIDGET SHELL ─────────────────────────────────────────────────────────────
// Spawn-size presets — used by App for initial dimensions; not for rendering.
...
function ResizeHandle({ type, cursor, style, onMouseDown }) {
  ...
  return (
    <div
      ...
      style={{
        position: 'absolute', cursor, zIndex: 12,
...
// ─── Widget header ────────────────────────────────────────────────────────────
function WidgetHeader({ title, isMaximized, onMaximize, isPinned, onPinToggle,
                        onMinimize, onDock, onClose, noClose, onMouseDown }) {
  return (
    <div className="widget-header" onMouseDown={onMouseDown}
      ...
      <div style={{ display: 'flex', gap: 4 }} className="no-drag">
        {!noClose && (
          <button className="icon-btn no-drag" title={isPinned ? 'Unpin' : 'Pin to toolbar'}
        ...
        {onDock && (
          <button className="icon-btn no-drag" title="Dock to console"
            onClick={(e) => { e.stopPropagation(); onDock?.() }}>
            <IconDock />
          </button>
        )}
        <button className="icon-btn no-drag" title={isMaximized ? 'Restore' : 'Maximize'}
          onClick={(e) => { e.stopPropagation(); onMaximize?.() }}>
          {isMaximized ? <IconRestore /> : <IconMaximize />}
        </button>
        <button className="icon-btn no-drag" title="Minimize"
          onClick={(e) => { e.stopPropagation(); onMinimize?.() }}><IconMinus /></button>
        {!noClose && (
          <button className="icon-btn no-drag" title="Close"
            onClick={(e) => { e.stopPropagation(); onClose?.() }}><IconX /></button>
        )}
      </div>
    </div>
  )
...
function WidgetBody({ children }) {
  return (
    <div style={{ height: 'calc(100% - 49px)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      {children}
    </div>
  )
}
...
export default function Widget({
  ...
  useEffect(() => {
    if (!isResizingRef.current) { setLocalW(width); setLocalH(height) }
  }, [width, height])

  // Init to spawn position on mount
  useEffect(() => {
    if (position) { xMV.set(position.x); yMV.set(position.y) }
  }, []) // eslint-disable-line
  ...
  const handleDragMouseDown = (e) => {
    ...
    e.preventDefault()
    const startX = xMV.get(), startY = yMV.get()
  ...
  const handleResizeMouseDown = (e, type) => {
    ...
    const onMove = (ev) => {
      ...
      setLocalW(nW); setLocalH(nH)
      xMV.set(nX); yMV.set(nY)
      latest.w = nW; latest.h = nH; latest.x = nX; latest.y = nY
    }
      ...
      <AnimatePresence>
        ...
        >
          <WidgetHeader title={title}
            isMaximized={isMaximized} onMaximize={onMaximize}
            isPinned={isPinned} onPinToggle={onPinToggle}
            onMinimize={onMinimize} onDock={onDock}
            onClose={onClose} noClose={noClose}
...
// ─── Icons ────────────────────────────────────────────────────────────────────
const IconX        = () => <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5"><line x1="1" y1="1" x2="11" y2="11"/><line x1="11" y1="1" x2="1" y2="11"/></svg>
const IconMinus    = () => <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5"><line x1="2" y1="6" x2="10" y2="6"/></svg>
const IconMaximize = () => <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M1 4V1h3M8 1h3v3M11 8v3H8M4 11H1V8" strokeLinecap="round" strokeLinejoin="round"/></svg>
const IconRestore  = () => <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M4 1.5v3H1M8 1.5h3v3M8 10.5h3v-3M4 10.5H1v-3" strokeLinecap="round" strokeLinejoin="round"/></svg>
const IconDock     = () => <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.2"><rect x="1.25" y="1.75" width="9.5" height="8.5" rx="1"/><line x1="7.5" y1="1.75" x2="7.5" y2="10.25"/><rect x="8" y="3" width="2" height="1.5" fill="currentColor" stroke="none"/><rect x="8" y="5.25" width="2" height="1.5" fill="currentColor" stroke="none"/></svg>
