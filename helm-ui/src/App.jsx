...
import Widget, { WIDGET_SIZES, WIDGET_MIN_W, WIDGET_MIN_H } from './components/Widget'
...
const COVERAGE_SHRINK  = 0.80
const COVERAGE_EVICT   = 0.90
...
const CONSOLE_DEFAULT_PCT    = 0.40
...
function computeCoverage(positions, dims, usable) {
  ...
  const xs = [...new Set(rects.flatMap(r => [Math.max(usable.left, r.x1), Math.min(usable.right,  r.x2)]))].sort((a, b) => a - b)
  const ys = [...new Set(rects.flatMap(r => [Math.max(usable.top,  r.y1), Math.min(usable.bottom, r.y2)]))].sort((a, b) => a - b)
...
export default function App() {
  ...
  const [pinnedWidgets,    setPinnedWidgets]  = useState(() => {
    try { return new Set(JSON.parse(localStorage.getItem('helm-pinned') || '[]')) }
    catch { return new Set() }
  })
  const [widgetLastConfig, setWidgetLastConfig] = useState(() => {
    try { return JSON.parse(localStorage.getItem('helm-last-config') || '{}') }
    catch { return {} }
  })
  ...
  const [dockPaneOpen,    setDockPaneOpen]    = useState(() => {
    try { return localStorage.getItem('helm-dock-pane-open') === '1' } catch { return false }
  })
  ...
  const usableRect = useMemo(() => {
    ...
    const consoleExtent = consoleState === 'collapsed'
  ...
  useEffect(() => { usableRectRef.current = usableRect }, [usableRect])
  ...
  useEffect(() => { widgetPositionsRef.current = widgetPositions }, [widgetPositions])
  useEffect(() => { widgetDimsRef.current      = widgetDims      }, [widgetDims])
  useEffect(() => { nodeOffsetRef.current      = nodeOffset      }, [nodeOffset])
  useEffect(() => { nodeShrunkenRef.current    = nodeShrunken    }, [nodeShrunken])
  useEffect(() => { minimizedRef.current       = minimized       }, [minimized])
  useEffect(() => { openWidgetsRef.current     = openWidgets     }, [openWidgets])
  ...
  useEffect(() => {
    const moved = prevNodeOffsetRef.current.x !== nodeOffset.x ||
    ...
    if (menuIntentRef.current && !nodeShrunken) {
      menuReopenTimer.current = setTimeout(() => {
        ...
        if (findNodeCoord(obs, nodeOffsetRef.current, usableRectRef.current, window.innerWidth, window.innerHeight) !== null) {
          setMenuOpen(true)
        }
      }, 950)
    }
  ...
  // ── Node adjustment ───────────────────────────────────────────────────────
  const adjustNode = useCallback((newPositions, newDims, curOffset, curShrunken, openWidgetsSnap, minimizedSnap, usable) => {
    ...
    const visiblePositions = Object.fromEntries(
      Object.entries(newPositions).filter(([id]) =>
        openWidgetsSnap.includes(id) && !minimizedIds.has(id)
      )
    )
    ...
    if (coverage >= COVERAGE_EVICT && openWidgetsSnap.length > 0) {
      ...
      const trimPos  = Object.fromEntries(Object.entries(newPositions).filter(([k]) => k !== oldestId))
      setOpenWidgets(prev => prev.filter(id => id !== oldestId))
      setMinimized(prev => prev.find(m => m.id === oldestId) ? prev : [...prev, { id: oldestId, title }])
      setWidgetPositions(trimPos)
      adjustNode(trimPos, newDims, curOffset, curShrunken,
        openWidgetsSnap.filter(id => id !== oldestId), minimizedSnap, usable)
      return
    }
    ...
    if (usable.bottom - usable.top < NODE_VP_MARGIN * 2 ||
    ...
    let centerClear = (
      usableCX >= usable.left + NODE_VP_MARGIN && usableCX <= usable.right  - NODE_VP_MARGIN &&
      usableCY >= usable.top  + NODE_VP_MARGIN && usableCY <= usable.bottom - NODE_VP_MARGIN
    )
    if (centerClear) {
      for (const obs of obstacles) {
        const nx = Math.max(obs.left, Math.min(usableCX, obs.right))
        const ny = Math.max(obs.top,  Math.min(usableCY, obs.bottom))
        if (Math.hypot(usableCX - nx, usableCY - ny) < NODE_CLEARANCE) { centerClear = false; break }
      }
    }
  ...
  const reflowWidgetsToUsable = useCallback((usable) => {
    ...
    for (const id of openIds) {
      ...
      if (tooSmall) { toMin.push(id); continue }
    ...
    if (toMin.length > 0) {
      for (const id of toMin) {
        const pos = positions[id], dim = dims[id]
    ...
    if (!tooSmall && autoIds.length > 0) {
      const restoreIds = autoIds.filter(id =>
        minIds.has(id) || minSnap.find(m => m.id === id)
      )
  ...
  const handleNodeMouseDown = useCallback((e) => {
    ...
    const startX  = nodeXMV.get(), startY = nodeYMV.get()
    ...
    const onMove = (ev) => {
      ...
      nodeDragRafRef.current = requestAnimationFrame(() => {
        ...
        const resolved = resolveNodeCollisions(ncx, ncy,
          widgetPositionsRef.current, widgetDimsRef.current, usableRectRef.current)
    ...
    const onUp = () => {
      ...
      if (hasDragged) {
        ...
        if (nodeDragRafRef.current) { cancelAnimationFrame(nodeDragRafRef.current); nodeDragRafRef.current = null }
        const fx = nodeXMV.get(), fy = nodeYMV.get()
  ...
  const startContemplate = useCallback(() => {
    ...
    setMenuOpen(false); menuIntentRef.current = false
    setNodeState('contemplating')
  }, [menuOpen])

  const endContemplate = useCallback(() => {
    setNodeState('idle')
    if (menuBeforeContemplate.current) { setMenuOpen(true); menuIntentRef.current = true }
    menuBeforeContemplate.current = false
  }, [])

  const handleNodeStateChange = useCallback((s) => {
  ...
  const computeSpawn = useCallback((id, angle, currentPositions, curOffset) => {
    ...
    const dim = widgetDimsRef.current[id] ?? (() => {
    ...
    const ncx  = vw / 2 + curOffset.x, ncy = vh / 2 + curOffset.y
    const ex   = ncx + Math.cos(rad) * r, ey = ncy + Math.sin(rad) * r
    ...
    const nodeCX = vw / 2 + curOffset.x, nodeCY = vh / 2 + curOffset.y
    const nodeObs = { x: nodeCX - NODE_CLEARANCE, y: nodeCY - NODE_CLEARANCE, w: NODE_CLEARANCE * 2, h: NODE_CLEARANCE * 2 }
    if (rectsOverlap({ x, y, w, h }, nodeObs)) {
      ...
      if (cleared) { x = cleared.x; y = cleared.y }
    }
    return { x, y }
  }, [])
  ...
  useEffect(() => {
    const onKey = (e) => { if (e.key === 'Escape') handleExitMaximize() }
  ...
  const handleMaximize = useCallback((id) => {
    if (maximizedWidget === id) { handleExitMaximize(); return }
    ...
    preMaximizeDimsRef.current       = { id, w: dim.w, h: dim.h, x: pos?.x ?? 0, y: pos?.y ?? usable.top }
    ...
    setNodeOffset({ x: NODE_ANCHOR_X - vw / 2, y: BAR_HEIGHT / 2 - vh / 2 })
  }, [maximizedWidget, openWidgets, minimized, handleExitMaximize, menuOpen])
  ...
  const handleMenuSelect = useCallback((id) => {
    ...
    if (openWidgets.includes(id)) {
      ...
      setOpenWidgets(newOpen); setWidgetPositions(newPos)
    ...
    if (coverage >= COVERAGE_EVICT && openWidgets.length > 0) {
      ...
      setMinimized(prev => prev.find(m => m.id === oldestId)
        ? prev : [...prev, { id: oldestId, title: WIDGET_DEFS[oldestId]?.title ?? oldestId }])
    }

    setWidgetPositions(newPos); setOpenWidgets(newOpen)
    adjustNode(newPos, widgetDims, nodeOffset, nodeShrunken, newOpen, minimized, usable)
  }, [minimized, openWidgets, widgetPositions, widgetDims, nodeOffset, nodeShrunken,
      startContemplate, endContemplate, computeSpawn, adjustNode])
  ...
  // ── Pill click ────────────────────────────────────────────────────────────
  const handlePillClick = useCallback((id, state) => {
    if (state === 'minimized') { handleRestore(id); return }
    const def = WIDGET_DEFS[id]
    if (!def) return
    const usable   = usableRectRef.current
  ...
  }, [widgetLastConfig, widgetDims, widgetPositions, openWidgets, nodeOffset, nodeShrunken,
      minimized, handleRestore, computeSpawn, adjustNode])
  ...
  const closeWidget = useCallback((id) => {
    ...
    setMinimized(p => p.filter(m => m.id !== id))
    setWidgetPositions(newPos)
    adjustNode(newPos, widgetDims, nodeOffset, nodeShrunken, newOpen, minimized, usableRectRef.current)
  ...
  const handleDockWidget = useCallback((id) => {
    ...
    const newPos  = Object.fromEntries(Object.entries(widgetPositionsRef.current).filter(([k]) => k !== id))
    ...
    if (!dockPaneOpen) {
      ...
      try { localStorage.setItem('helm-dock-pane-open', '1') } catch {}
    }
    // Re-center node against updated canvas
    adjustNode(newPos, widgetDimsRef.current, nodeOffsetRef.current,
      nodeShrunkenRef.current, newOpen, newMin, usableRectRef.current)
  }, [dockPaneOpen, adjustNode])

  const handleUndockWidget = useCallback((id) => {
    const def = WIDGET_DEFS[id]
    if (!def) return
    ...
    setWidgetPositions(newPos)
    setOpenWidgets(newOpen)
    adjustNode(newPos, newDims, nodeOffsetRef.current,
      nodeShrunkenRef.current, newOpen, minimizedRef.current, usableRectRef.current)
  ...
  const openWidgetById = useCallback((id) => {
    if (!WIDGET_DEFS[id]) return
    if (dockedWidgets.includes(id))                  { handleUndockWidget(id); return }
    if (minimizedRef.current.find(m => m.id === id)) { handleRestore(id); return }
    if (openWidgetsRef.current.includes(id)) return   // already visible

    const usable = usableRectRef.current
    const item   = MENU_ITEMS.find(m => m.id === id)
    const spawnPos = computeSpawn(id, item?.angle ?? 0,
      widgetPositionsRef.current, nodeOffsetRef.current)
    const newPos  = { ...widgetPositionsRef.current, [id]: spawnPos }
    const newOpen = [...openWidgetsRef.current, id]
    setWidgetPositions(newPos)
    setOpenWidgets(newOpen)
    adjustNode(newPos, widgetDimsRef.current, nodeOffsetRef.current,
      nodeShrunkenRef.current, newOpen, minimizedRef.current, usable)
  }, [dockedWidgets, handleUndockWidget, handleRestore, computeSpawn, adjustNode])

  // ── Widget resize ─────────────────────────────────────────────────────────
  const handleWidgetResizeEnd = useCallback((id, w, h, x, y) => {
    widgetTargetDimsRef.current[id] = { w, h }
    const newDims = { ...widgetDimsRef.current, [id]: { w, h } }
    ...
    setWidgetPositions(newPos)
    adjustNode(newPos, newDims, nodeOffsetRef.current,
      nodeShrunkenRef.current, openWidgets, minimized, usableRectRef.current)
  ...
  const handleWidgetDragEnd = useCallback((id, x, y) => {
    setActiveWidget(null)
    const usable = usableRectRef.current
    const d  = getDim(id, widgetDimsRef.current)
    const cx = Math.max(usable.left + SPAWN_MARGIN, Math.min(x, usable.right  - d.w - SPAWN_MARGIN))
    const cy = Math.max(usable.top  + SPAWN_MARGIN, Math.min(y, usable.bottom - d.h - SPAWN_MARGIN))
    const newPos = { ...widgetPositionsRef.current, [id]: { x: cx, y: cy } }
    setWidgetPositions(newPos)
    widgetPositionsRef.current = newPos
    adjustNode(newPos, widgetDimsRef.current, nodeOffsetRef.current,
      nodeShrunkenRef.current, openWidgets, minimized, usable)
  }, [openWidgets, minimized, adjustNode])
  ...
  const handleNodeClick = useCallback(() => {
    ...
    if (nodeShrunken) {
      ...
      if (maximizedWidget) { handleExitMaximize(); return }
      ...
      if (consoleState !== 'standard') {
        ...
        if      (consolePosition === 'bottom') usable = { left: 0, top: BAR_HEIGHT, right: vpW, bottom: Math.max(BAR_HEIGHT, vpH - extent) }
        else if (consolePosition === 'left')   usable = { left: Math.min(vpW, extent), top: BAR_HEIGHT, right: vpW, bottom: vpH }
      ...
      const newPos  = Object.fromEntries(Object.entries(widgetPositions).filter(([k]) => !toClose.includes(k)))
    ...
    setMenuOpen(m => !m)
  }, [nodeShrunken, maximizedWidget, handleExitMaximize, widgetPositions, widgetDims, openWidgets, menuOpen,
      consoleState, consoleSizePct, consolePosition, vpW, vpH])
  ...
  const handleConsolePositionChange = useCallback((p) => {
    ...
    try { localStorage.setItem('helm-console-position', p) } catch {}
  }, [])
  ...
  const handleDockPaneResize = useCallback((px) => {
    setDockPaneWidth(px)
    try { localStorage.setItem('helm-dock-pane-width', String(px)) } catch {}
  }, [])
  const handleDockPaneToggle = useCallback(() => {
    setDockPaneOpen(prev => {
      const next = !prev
      try { localStorage.setItem('helm-dock-pane-open', next ? '1' : '0') } catch {}
      return next
    })
  }, [])
  ...
  useEffect(() => {
    try { localStorage.setItem('helm-docked-widgets', JSON.stringify(dockedWidgets)) } catch {}
  }, [dockedWidgets])
  ...
  const consoleRect = useMemo(() => {
    ...
    const extent = consoleState === 'collapsed'
      ? CONSOLE_COLLAPSED_PX
      : Math.round(consoleSizePct * (consolePosition === 'bottom' ? vpH : vpW))
  ...
  const navPills = useMemo(() => {
    const minimizedPills = minimized.map(({ id, title }) => ({
      id, title, state: 'minimized', pinned: pinnedWidgets.has(id),
    }))
